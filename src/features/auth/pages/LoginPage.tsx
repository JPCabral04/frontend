...
export function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      setApiError(msg === 'Invalid credentials' ? 'Email ou senha incorretos.' : 'Erro ao conectar. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box sx={styles.container}>
      ...
      <Box sx={styles.card} className="animate-scaleIn">
        ...
        <Typography sx={styles.registerLink}>
          Não tem uma conta?{' '}
          <Link to="/auth/register" style={{ color: 'hsl(245, 85%, 65%)', fontWeight: 600 }}>
            Criar conta
          </Link>
        </Typography>
      </Box>
